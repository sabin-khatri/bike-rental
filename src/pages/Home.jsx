...
export default function Home() {
  const bikes = [
    { name: "Royal Enfield Classic 350", price: "Rs 800", tag: "Cruiser", img: royalEnfield },
    { name: "Honda XR 190", price: "Rs 1200", tag: "Off-Road", img: hondaXr },
    { name: "Bajaj Pulsar 220", price: "Rs 700", tag: "Street", img: bajajPulsar }
  ];

  const features = [
    { title: "Fully Insured", desc: "Every rental includes comprehensive insurance – ride worry-free.", icon: <IconShield /> },
    { title: "24/7 Support", desc: "Mechanical help anytime, anywhere in Nepal.", icon: <IconClock /> },
    { title: "Latest Models", desc: "2023–2025 fleet, serviced after every ride.", icon: <IconZap /> },
    { title: "Free Delivery", desc: "Delivered to your hotel in Thamel or Pokhara – free of cost.", icon: <IconMapPin /> }
  ];
    ...
    <div className="bg-slate-50 min-h-screen font-sans">
        ...
        <div className="relative z-10 text-center px-6 max-w-6xl mx-auto">
          <motion.div variants={stagger} initial="hidden" animate="visible">
            ...
            <motion.div variants={fadeInUp} className="mt-12 flex flex-col sm:flex-row gap-6 justify-center">
              <Link
                to="/bikes"
                className="group relative px-10 py-5 bg-cyan-500 text-black font-bold text-lg rounded-full overflow-hidden shadow-xl hover:shadow-cyan-500/40 transition-all hover:scale-105"
              >
                <span className="relative z-10 flex items-center gap-3">
                  Find Your Bike <IconArrowRight />
                </span>
              ...
              <Link
                to="/contact"
                className="px-10 py-5 border border-white/30 bg-white/10 backdrop-blur-sm text-white font-medium rounded-full hover:bg-white/20 transition-all flex items-center justify-center gap-3"
              >
      ...
      <div className="relative -mt-20 z-20">
        <div className="max-w-6xl mx-auto bg-white rounded-3xl shadow-2xl p-8 md:p-12 grid grid-cols-2 md:grid-cols-4 gap-8 divide-y md:divide-y-0 md:divide-x divide-gray-100">
          {["500+", "50+", "100%", "24/7"].map((num, i) => (
            <div key={i} className="text-center">
              <h3 className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-b from-gray-900 to-gray-700">
                {num}
              </h3>
              <p className="mt-2 text-sm font-bold text-gray-500 uppercase tracking-wider">
                {["Happy Riders", "Premium Bikes", "Insurance Cover", "Road Support"][i]}
              </p>
            </div>
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {bikes.map((bike, i) => (
              <motion.div
                ...
                transition={{ delay: i * 0.1 }}
                className="group relative h-96 md:h-[520px] rounded-3xl overflow-hidden bg-gray-800 shadow-2xl cursor-pointer"
              >
                <img
                  src={bike.img}
                  alt={bike.name}
                ...
                <div className="absolute bottom-8 left-8 right-8 text-white translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                  <h3 className="text-2xl font-bold mb-2 group-hover:text-cyan-400 transition-colors">
                    {bike.name}
                  </h3>
                  <div className="flex justify-between items-end">
                    <div>
                      <span className="text-3xl font-black text-cyan-400">{bike.price}</span>
                      <span className="text-sm text-gray-400 ml-1">/day</span>
                    </div>
                    <Link
                      to="/bikes"
                      className="opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-300 bg-cyan-500 text-black px-6 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-cyan-400"
                    >
      ...
      <section className="py-24 bg-cyan-50/50">
        <div className="max-w-4xl mx-auto text-center px-6">
          <IconStar />
          <h2 className="mt-8 text-3xl md:text-4xl font-bold text-gray-900 italic">
      ...
      <section className="py-32 bg-gradient-to-br from-gray-900 to-blue-900 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-4xl mx-auto px-6">
          ...
          <Link
            to="/bikes"
            className="mt-12 inline-block px-12 py-6 bg-white text-blue-900 font-bold text-xl rounded-full shadow-2xl hover:shadow-cyan-300/50 hover:scale-105 transition-all duration-300"
          >
            Book Your Bike Now
          </Link>
        </div>
